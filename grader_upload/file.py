...
if __name__ == "__main__":
    def self_checks() -> None:
        ...
        status = tracker.add("https://example.com", "h1")
        assert status == 1
        status = tracker.add("https://example.com", "h1")
        assert status == 0
        status = tracker.add("https://example.com", "p")
        assert status == 2

        html = "<html><body><h1>Title</h1><p>Alpha <span>Beta</span></p></body></html>"
        result = tracker.extract_from_html("https://example.com", html)
        assert result["h1"] == ["Title"]
        assert result["p"] == ["Alpha Beta"]

        assert tracker.remove_selector("https://example.com", "h1") is True
